...
// Commands for the Control/Status register.
//Comandos para el Registro de Control/Estado
#define PCF8583_START_COUNTING_WITH_ALARM 0x04
#define PCF8583_STOP_COUNTING_WITH_ALARM 0x84

//Commands for the alarm control register
//Comando para el registro de control de alarma
#define PCF8583_DAILY_ALARM_INTERRUPTION_ENABLE 0x97
...
// Equal previous case, we define a user's alarm date.
// The values are stored as unsigned integers.
// Como en el caso anterior, definimos una fecha de alarma.
// Los valores son almacenado como tipo unsigned int.

typedef struct{
   int8 seconds;    // 0 to 59
   int8 minutes;    // 0 to 59
   int8 hours;      // 0 to 23  (24-hour time)
   //Day and Month ignored for daily alarm case\\Día y mes Ignorado para el caso de alarma diaria
}date_alarm_t;
...
//With alarm
//Con Alarma
void PCF8583_init_al(void){
   PCF8583_write_byte(PCF8583_CTRL_STATUS_REG, PCF8583_START_COUNTING_WITH_ALARM);
   PCF8583_write_byte(PCF8583_ALARM_CONTROL_REG, PCF8583_DAILY_ALARM_INTERRUPTION_ENABLE);
}
...
void PCF8583_set_datetime(date_time_t *dt)
{
   ...
   PCF8583_write_byte(PCF8583_CTRL_STATUS_REG, PCF8583_STOP_COUNTING_WITH_ALARM);
...
//----------------------------------------------
void PCF8583_set_datealarm(date_alarm_t *al)
{
int8 bcd_sec;
int8 bcd_min;
int8 bcd_hrs;
...
// Convert the input date/time into BCD values
// that are formatted for the PCF8583 registers.
bcd_sec = bin2bcd(al->seconds);
bcd_min = bin2bcd(al->minutes);
bcd_hrs = bin2bcd(al->hours);
...
i2c_start();
i2c_write(PCF8583_WRITE_ADDRESS);
i2c_write(PCF8583_ALARM_100S_REG);   // Start at 100's reg.
i2c_write(0);               // Set 100's reg = 8
i2c_write(bcd_sec);
i2c_write(bcd_min);
i2c_write(bcd_hrs);
i2c_stop();
...
void PCF8583_read_datealarma(date_alarm_t *al)
{
...
i2c_start();
i2c_write(PCF8583_WRITE_ADDRESS);
i2c_write(PCF8583_ALARM_MINS_REG);   // Start at minutes reg.
i2c_start();
i2c_write(PCF8583_READ_ADDRESS);
...
// unsigned 8-bit integers.  Unpack the bits
// in the PCF8583 registers where required.
al->minutes = bcd2bin(bcd_min);
al->hours   = bcd2bin(bcd_hrs & 0x3F);
}
